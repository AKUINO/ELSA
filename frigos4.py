...
def alertOwner(currSensor):
        ...
        if currSensor.alarm == None:
                ...
                message = message + u" enfreint "+currSensor.alarm #+u"="+str(defins[transac['alarm']])
        print message
                                                ...
                                                if len(line) == 10:
                                                        ...
                                                        for sensor in c.AllSensors.elements:
                                                            ...
                                                            if (currSensor.fields['sensor'].translate(None, '. ') == HEX.translate(None, '. ')):
                                                                ...
                                                                StepValuesUpdate(currSensor, str(temperature))
                                                line = None
                                        else:
                                                line.append(data)
                        except:
...
class InputEventThread(threading.Thread):
    ...
    def __init__(self,currScanner):
        ...
        self.dev = evdev.InputDevice("/dev/input/event"+str(self.scanner.numDev))
                        ...
                        if data.scancode == 42:
                            ...
                            if data.scancode == 28:
                                ...
                                with screen.lock:
                                    ...
                                    screen.refreshDisplay = isNow+datetime.timedelta(seconds=12)
                                    contexte.BarcodeIn(res[:13],screen,isNow,bluetooth)
                                res=''
                            else:
                                #print(data.scancode)
                                try:
                        ...
                        if capab[:5] == 'event':
                            ...
                            if physical:
                                ...
                                if key in c.AllScanners.elements:
                                    ...
                                    screen.devConnected += str(currScanner.rank)
                                    ...
                                    if not somethingDisplayed:
                                        ...
                                        screen.draw.rectangle((screen.begScreen,0,131,63),fill=0)
                                        screen.linePos = 0
                                    ...
                                    screen.draw.text((screen.begScreen,screen.linePos+1), "Connect "+str(currScanner.rank)+"#"+str(currScanner.id), font=screen.font,fill=255)
                                    screen.linePos += screen.lineHeight
                                    screen.draw.text((screen.begScreen,screen.linePos+1), "            "+currScanner.mac, font=screen.font,fill=255)
                                    screen.linePos += screen.lineHeight
...
def eventDisplay(screen,strid,timestamp,value,timespan,alarm):
        ...
        if not timespan == None:
                draw.text((84,screen.linePos+1), strDelta(timespan), font=screen.font,fill=255)
        if not alarm == None:
                screen.draw.text((123,screen.linePos), symbols[aAlarm], font=screen.fontG10, fill=255)
                screen.draw.text((102,screen.linePos+1), alarm, font=screen.font, fill=255)
                    ...
                    for nSensor in c.AllSensors.elements:
                            ...
                            if hasattr(currSensor,'data'):
                                ...
                                eventDisplay (screen, currSensor.fields['acronym'],
                                              currSensor.timestamp if hasattr(currSensor,'timestamp') else "",
