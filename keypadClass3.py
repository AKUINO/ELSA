...
class keypad():
    ...
    def __init__(self, gpio=None):
        self.GPIO = GPIO(pinmode=1)  # WPI_MODE_GPIO
        # Set all columns as output low
        for j in range(len(self.COLUMN)):
            self.GPIO.pinMode(self.COLUMN[j], GPIO.OUTPUT)
            self.GPIO.digitalWrite(self.COLUMN[j], 0)

        for i in range(len(self.ROW)):
            self.GPIO.pinMode(self.ROW[i], GPIO.INPUT)
            self.GPIO.pullUpDnControl(self.ROW[i], GPIO.PUD_DOWN)
        print('Init done')
    ...
    def getKey(self):
        ...
        for j in range(len(self.COLUMN)):
            colVal = j
            self.GPIO.digitalWrite(self.COLUMN[j], 1)
            sleep(0.01)
            rowVal = -1
            for i in range(len(self.ROW)):
                # Scan rows for pushed key/button
                # A valid key press should set "rowVal"  between 0 and 3.
                tmpRead = self.GPIO.digitalRead(self.ROW[i])
                if tmpRead != 0:
                    if rowVal < 0:
                        rowVal = i
                    else:
