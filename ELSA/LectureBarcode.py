...
class BarcodeReader():
    ...
    def BarcodeIn(self, i, screen, timestamp, bluetooth):
        ...
        screen.draw.text((4, screen.linePos+1), strnow,
                         font=screen.font, fill=255)
        strid = str(i)
        screen.draw.rectangle((28, screen.linePos, 30+screen.draw.textsize(
            strid, font=screen.font)[0], screen.linePos+10), fill=255)
        screen.draw.text((30, screen.linePos+1), strid,
                         font=screen.font, fill=0)
        ...
        if(i in self.c.barcode):
            ...
            if(barcodeClass == Measure and self.user != ""):
                ...
                screen.draw.text(
                    (4, screen.linePos+1), self.user.context.measure.fields['name'], font=screen.font, fill=255)
                screen.linePos += screen.lineHeight
                ...
                for step in recipeSteps:
                    if (recipeSteps[step].fields['p_id'] == piecei) and (recipeSteps[step].fields['e_id'] == equipmenti) and (recipeSteps[step].fields['h_id'] == phasei):
                        if(int(recipeSteps[step].fields['seq']) > self.currBatch.oldSeq):
                            ...
                            self.currBatch.currStep = self.c.AllRecipes.elements[
                                self.currBatch.fields['r_id']].AllSteps.elements[step]
            ...
            screen.draw.rectangle(
                (28, 0, 32+screen.draw.textsize(strid, font=screen.font)[0], 10), fill=255)
