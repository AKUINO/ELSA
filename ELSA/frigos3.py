...
typeNames = [u"?", u"Alerte", u"?", u"?",
             u"Lieu", u"Mesure", u"Personne", u"Lot"]
...
c = Configuration()
c.load()
print c.AllUsers.elements

bc = BarcodeReader(c)


def StepValuesUpdate(sensorIn, value):
    currSensor = None
    for sensor in c.AllSensors.elements:
        if (c.AllSensors.elements[sensor].fields['sensor'].translate(None, '.') == sensorIn.translate(None, '.')):
            ...
            print(u"Sensor ID  : " + currSensor.fields['sensor'])
            print(u"Sensor name: " + currSensor.fields['name'])
    if(currSensor != None):
...
def alertOwner(HEX):
    ...
    if idDefinitions.has_key(HEX):
        ...
        message = typeNames[defins['typed']]+u" " + \
            defins['name']+u"="+str(transac['data'])
        if transac['alarm'] == None:
            message = message + u" OK"
        else:
            message = message + u" enfreint " + \
                transac['alarm']+u"="+str(defins[transac['alarm']])
        for nBadge, aBadge in badging.iteritems():
            if aBadge['incoming']:
                ...
                if idDefinitions.has_key(nBadge):
                    ...
                    message = message + \
                        typeNames[defins['typed']]+u" "+defins['name']
                else:
                ...
                if data == '[':
                    ...
                    if data == ']':
                        if len(line) == 10:
                            ...
                            print(str(HEX) + " = " + str(temperature))
                            StepValuesUpdate(str(HEX), str(temperature))
                        line = ''.join(line)
                        print(line)
        ...
        while ALIVE:
            ...
            for aDevice in owDevices.sensorList():
                try:
                    ...
                    print(owaddress + " = " + owtemperature + " deg.C")
                    StepValuesUpdate(owaddress, owtemperature)
        ...
        if (dev is None):
            try:
                dev = evdev.InputDevice(
                    "/dev/input/by-id/usb-Barcode_AFANDA_BARCODE_AFANDBARCODE-event-kbd")
                print(dev)
                dev.grab()
                print("grabbed!")
            except:
            ...
            try:
                for event in dev.read():
                    ...
                    if isinstance(data, evdev.KeyEvent):
                        if data.scancode == 42:
                            caps = False
                        if data.keystate == 1:
                            if data.scancode == 28:
                ...
                if anError.errno == 11:
                    ...
                    try:
                        dev.ungrab()
                        print("ungrabbed " + str(anError.errno))
                    except:
                        pass
                    dev = None
            except:
...
threadbcRead = threading.Thread(target=bcRead)
threadbcRead.start()
    ...
    while ALIVE:
        ...
        for nBadge, aBadge in badging.iteritems():
            eventDisplay(linePos, nBadge, aBadge['timestamp'],
                         u"IN" if aBadge['incoming'] else u"out", aBadge['timespan'], None)
            linePos += lineHeight
